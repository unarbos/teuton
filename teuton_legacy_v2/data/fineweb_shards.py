...
CLI:
    ...
    python -m teuton.data.fineweb_shards prepare \
        --target-tokens 1_000_000_000 \
        --shard-tokens  1_048_576

    # Inspect what's on the bucket
    python -m teuton.data.fineweb_shards inspect

    # Re-upload only missing shards (idempotent)
    python -m teuton.data.fineweb_shards prepare --resume
"""
...
def _build_bucket() -> LocalBucket | S3Bucket:
    bucket_name = os.environ.get("S3_BUCKET") or os.environ.get("TEUTON_S3_BUCKET")
    if bucket_name:
        return S3Bucket(
            bucket=bucket_name,
    ...
    root = os.environ.get("TEUTON_LOCAL_BUCKET", "/tmp/teuton_bucket")
    return LocalBucket(root=root, bucket=os.environ.get("TEUTON_BUCKET", "teuton-test"))
...
def main(argv: list[str] | None = None) -> int:
    ...
    sp.add_argument("--cache-path", default="/tmp/teuton_data/fineweb_edu_tokens.i32",
