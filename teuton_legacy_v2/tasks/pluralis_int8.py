"""Stage 10 — int8 per-channel quantization of the projected wire.

Builds on Stage 8 (lossless wire). The projected (B, T, k) coefficients are
quantized to int8 per-channel before write; the inner step dequantizes
before unprojecting.

Schedule pairs the (q, scale) outputs at distinct URIs because schedule.py's
per-output mapping is by `name`. We pack q into the standard target URI
and write the per-channel scale at a separate "_scale.bin" URI.

For Stage 10, we only quantize UB-1's wire (which is already projected).
"""
...
def build_forward_graph(*, bucket: str, run_id: str) -> Graph:
    """Forward outputs:
       UB-0: full-rank (2, B, T, d) target_0
       UB-1: int8-quantized (2, B, T, k) target_1 + fp32 scale "scale_1"
    """
    gb = GraphBuilder()
    ...
    h_1_proj = gb.matmul(h_1, u_k)
    target_1_proj = gb.matmul(target_1, u_k)
    pack_1_fp32 = gb.stack([h_1_proj, target_1_proj], dim=0)   # (2, B, T, k) fp32

    # int8 per-channel quantize along last axis
    q, scale = gb.quantize_int8_per_channel(pack_1_fp32, dim=-1)

    # Dummy scale_0 for UB-0 (unused but required by schedule's per-UB extras)
    dummy_scale_0 = gb.unsqueeze(gb.const(0.0), dim=0)   # shape (1,)

    gb.output("target_0", pack_0)
    gb.output("target_1", q)         # int8 (2, B, T, k)
    gb.output("scale_0", dummy_scale_0)
    gb.output("scale_1", scale)      # fp32 (2, B, T, 1)
    return gb.build()
...
def build_inner_graph_ub1(*, bucket: str, run_id: str) -> Graph:
    """UB-1: dequantize + unproject + K-step SGD + project."""
    gb = GraphBuilder()
    weights = gb.input("weights", [D, D], DTYPE)
    q = gb.input("target", [2, B_TRAIN, T, SUBSPACE_K], "int8")
    # scale: per-channel fp32 with shape (2, B, T, 1) due to quantize op output
    scale = gb.input("scale", [1, 1, 1, SUBSPACE_K], DTYPE)
    u_k = gb.const_blob(
        f"s3://{bucket}/{paths.static_blob_key(run_id, 'U_k')}",
        shape=[D, SUBSPACE_K], dtype=DTYPE,
    )

    pack_proj = gb.dequantize_int8_per_channel(q, scale)   # (2, B, T, k) fp32
    u_k_t = gb.transpose(u_k, dims=[1, 0])
    packed = gb.matmul(pack_proj, u_k_t)                   # (2, B, T, d)
...
def bootstrap(*, bucket: LocalBucket, run_id: str, max_rounds: int) -> None:
    ...
    bucket.put_json(
        ...
        {"task": "pluralis_int8", "n_unique_blocks": N_UB, "d": D, "k": SUBSPACE_K,
...
def build_orchestrator_inputs(
    *, bucket: LocalBucket, run_id: str
    ...
    # forward emits an extra `scale_1` output; UB-1's inner_step needs the
    # `scale` input. Wire these via forward_extra_outputs (creates a per-cut
    # extra URI; for cut=1 it's the scale, for cut=0 it's unused).
