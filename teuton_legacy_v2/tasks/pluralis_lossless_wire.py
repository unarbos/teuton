"""Stage 8 — Pluralis lossless wire projection.

Builds on Stage 7. UB-1's W1 is constrained to a rank-k subspace S; thus
its block input `h_1` and target `target_1 = h_2 - dL/dh_2` both lie in S.
The forward graph projects them onto S via U_k, shipping shape
(B, T, k) instead of (B, T, d) — a lossless k/d × wire reduction. The
inner_step graph for UB-1 receives the projected tensor and unprojects
back to (B, T, d) before the K-step SGD.

Per-UB inner graphs are used: UB-0 receives full-rank (B, T, d), UB-1
receives the projected (B, T, k).

Stage-8 deliverable: the per-UB target.bin file size for UB-1 drops to
~k/d × the Stage 7 size, while CE loss continues to decrease.
"""
...
# --------------------------------------------------------------------------- #
# Static blobs (same as pluralis_subspace)
# --------------------------------------------------------------------------- #
...
def build_forward_graph(*, bucket: str, run_id: str) -> Graph:
    """Forward emits per-UB packed targets:
       - UB-0 (unconstrained): packed shape (2, B, T, d)
       - UB-1 (subspace-constrained): packed projected to S, shape (2, B, T, k).
    """
    gb = GraphBuilder()
    ...
    target_0 = gb.sub(h_1, dL_dh_1)        # (B, T, d)
    target_1 = gb.sub(h_2, dL_dh_2)        # (B, T, d) — but lives in S

    # UB-0: full-rank (B, T, d) packed
    pack_0 = gb.stack([h_0, target_0], dim=0)         # (2, B, T, d)

    # UB-1: project x_in (h_1) and target onto S -> (B, T, k)
    h_1_proj = gb.matmul(h_1, u_k)                    # (B, T, k)
    target_1_proj = gb.matmul(target_1, u_k)          # (B, T, k)
...
def build_inner_graph_ub0() -> Graph:
    """UB-0: full-rank inner step (no subspace involvement)."""
    gb = GraphBuilder()
...
def build_inner_graph_ub1(*, bucket: str, run_id: str) -> Graph:
    """UB-1: receives projected (2, B, T, k); unprojects via U_k^T, runs SGD,
    re-projects W1 to keep it in S."""
    gb = GraphBuilder()
    weights = gb.input("weights", [D, D], DTYPE)
    ...
    # Unproject: c @ U_k^T -> (2, B, T, d)
    u_k_t = gb.transpose(u_k, dims=[1, 0])              # (k, d)
    packed = gb.matmul(packed_proj, u_k_t)              # (2, B, T, d)
...
def bootstrap(*, bucket: LocalBucket, run_id: str, max_rounds: int) -> None:
    ...
    bucket.put_json(
        ...
        {"task": "pluralis_lossless_wire", "n_unique_blocks": N_UB, "d": D, "k": SUBSPACE_K,
...
def build_orchestrator_inputs(
    *, bucket: LocalBucket, run_id: str
    ...
    graphs = TaskGraphs(
        ...
        inner=inner_ub0,                       # default = ub0; overridden via per_ub
