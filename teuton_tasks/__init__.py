"""Task plugin registry for Teuton v3."""
...
def load_task(name: str):
    if name in STREAMING_TASKS:
        raise ValueError(
            ...
            "under teuton_tasks and should be run through StreamingRunManager."
        )
    if name in LEGACY_ROUND_TASKS:
        raise ValueError(
            f"task {name!r} is preserved from v2 under teuton_tasks.{name}, but "
            "the v3 RunManager supports only native round tasks. Use the "
            "`teuton-v2-legacy` runner for v2-style round orchestration."
        )
    if name not in KNOWN_TASKS:
        known = sorted(KNOWN_TASKS | STREAMING_TASKS)
        raise ValueError(f"unknown v3 task {name!r}; known={known}")
    return importlib.import_module(f"teuton_tasks.{name}")
