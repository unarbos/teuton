"""Small self-contained tensor IR used by Teuton v3."""
...
def ref_const_blob(uri: str, shape: Sequence[int], dtype: str) -> dict[str, Any]:
    return {"kind": "const_blob", "uri": uri, "shape": [int(s) for s in shape], "dtype": dtype}
...
for _n in ("add", "sub", "mul", "div", "pow"):
    _register(_n, 2)
for _n in ("neg", "exp", "log", "sqrt", "abs", "round", "relu", "gelu", "silu", "sin", "cos", "sigmoid", "tanh", "identity"):
    _register(_n, 1)
...
_register("topk", 1, kwargs=("k", "dim"))
_register("softmax", 1, kwargs=("dim",))
_register("log_softmax", 1, kwargs=("dim",))
_register("layer_norm", 3, kwargs=("eps",))
_register("rmsnorm", 2, kwargs=("eps",))
_register("cross_entropy", 2, kwargs=("ignore_index",))
_register("embedding", 2)
_register("tril", 1, kwargs=("diagonal",))
_register("triu", 1, kwargs=("diagonal",))
_register("full", 0, kwargs=("shape", "value", "dtype"))
_register("quantize_int8_per_channel", 1, kwargs=("dim",))
_register("dequantize_int8_per_channel", 2)
_register("quantize_pack_int8", 1, kwargs=("dim",))
_register("unpack_dequantize_int8", 1, kwargs=("dim", "shape", "scale_dim"))
_register("qr", 1)
...
class GraphBuilder:
    ...
    def emit(self, op: str, args: Sequence[dict[str, Any]] = (), kwargs: dict[str, Any] | None = None, out: Sequence[TensorSpec] = ()) -> dict[str, Any]:
        sig = op_signature(op)
        if sig.n_args >= 0 and len(args) != sig.n_args:
            raise ValueError(f"op {op!r} expects {sig.n_args} args, got {len(args)}")
        kw = dict(kwargs or {})
        for k in kw:
            if k not in sig.kwargs:
                raise ValueError(f"op {op!r} got unexpected kwarg: {k!r}")
        op_id = len(self._ops)
        self._ops.append(Op(id=op_id, op=op, args=[dict(a) for a in args], kwargs=kw, out=list(out)))
        return ref_op(op_id, idx=0)

    def emit_multi(self, op: str, args: Sequence[dict[str, Any]] = (), kwargs: dict[str, Any] | None = None, n_outputs: int = 2) -> tuple[dict[str, Any], ...]:
        sig = op_signature(op)
        if sig.n_args >= 0 and len(args) != sig.n_args:
            raise ValueError(f"op {op!r} expects {sig.n_args} args, got {len(args)}")
        kw = dict(kwargs or {})
        for k in kw:
            if k not in sig.kwargs:
                raise ValueError(f"op {op!r} got unexpected kwarg: {k!r}")
        op_id = len(self._ops)
        self._ops.append(Op(id=op_id, op=op, args=[dict(a) for a in args], kwargs=kw, out=[]))
    ...
    def sum(self, a, *, dim=None, keepdim=False): return self.emit("sum", [a], kwargs={"dim": dim, "keepdim": bool(keepdim)})
    def mean(self, a, *, dim=None, keepdim=False): return self.emit("mean", [a], kwargs={"dim": dim, "keepdim": bool(keepdim)})
    def max_(self, a, *, dim=None, keepdim=False): return self.emit("max", [a], kwargs={"dim": dim, "keepdim": bool(keepdim)})
    def min_(self, a, *, dim=None, keepdim=False): return self.emit("min", [a], kwargs={"dim": dim, "keepdim": bool(keepdim)})
    def concat(self, tensors, *, dim): return self.emit("concat", list(tensors), kwargs={"dim": int(dim)})
    def stack(self, tensors, *, dim): return self.emit("stack", list(tensors), kwargs={"dim": int(dim)})
    def split(self, a, *, sizes, dim): return self.emit_multi("split", [a], kwargs={"sizes": list(sizes), "dim": int(dim)}, n_outputs=len(sizes))
    def broadcast(self, a, *, shape): return self.emit("broadcast", [a], kwargs={"shape": list(shape)})
    def squeeze(self, a, *, dim): return self.emit("squeeze", [a], kwargs={"dim": int(dim)})
    def unsqueeze(self, a, *, dim): return self.emit("unsqueeze", [a], kwargs={"dim": int(dim)})

    def slice(self, a, *, dim, start, end):
        return self.emit("slice", [a], kwargs={"dim": int(dim), "start": start if isinstance(start, dict) else int(start), "end": end if isinstance(end, dict) else int(end)})

    def gather(self, a, index, *, dim): return self.emit("gather", [a, index], kwargs={"dim": int(dim)})
    def scatter(self, a, index, src, *, dim): return self.emit("scatter", [a, index, src], kwargs={"dim": int(dim)})
    def arange(self, *, start, end, step=1, dtype="int64"): return self.emit("arange", [], kwargs={"start": start, "end": end, "step": step, "dtype": dtype})
    def normal(self, *, seed, shape, dtype="float32"): return self.emit("normal", [], kwargs={"seed": seed, "shape": list(shape), "dtype": dtype})
    def uniform(self, *, seed, shape, dtype="float32"): return self.emit("uniform", [], kwargs={"seed": seed, "shape": list(shape), "dtype": dtype})
    def sort(self, a, *, dim=-1, descending=False): return self.emit("sort", [a], kwargs={"dim": int(dim), "descending": bool(descending)})
    def topk(self, a, *, k, dim=-1): return self.emit_multi("topk", [a], kwargs={"k": int(k), "dim": int(dim)}, n_outputs=2)
    def softmax(self, a, *, dim=-1): return self.emit("softmax", [a], kwargs={"dim": int(dim)})
    def log_softmax(self, a, *, dim=-1): return self.emit("log_softmax", [a], kwargs={"dim": int(dim)})
    def layer_norm(self, a, weight, bias, *, eps=1e-5): return self.emit("layer_norm", [a, weight, bias], kwargs={"eps": float(eps)})
    def rmsnorm(self, a, weight, *, eps=1e-5): return self.emit("rmsnorm", [a, weight], kwargs={"eps": float(eps)})
    def tril(self, a, *, diagonal=0): return self.emit("tril", [a], kwargs={"diagonal": int(diagonal)})
    def triu(self, a, *, diagonal=0): return self.emit("triu", [a], kwargs={"diagonal": int(diagonal)})
    def full(self, *, shape, value, dtype="float32"): return self.emit("full", [], kwargs={"shape": list(shape), "value": value, "dtype": dtype})
    def cross_entropy(self, logits, targets, *, ignore_index=-100): return self.emit("cross_entropy", [logits, targets], kwargs={"ignore_index": int(ignore_index)})
    def embedding(self, weight, ids): return self.emit("embedding", [weight, ids])
    def quantize_int8_per_channel(self, x, *, dim=-1): return self.emit_multi("quantize_int8_per_channel", [x], kwargs={"dim": int(dim)}, n_outputs=2)
    def dequantize_int8_per_channel(self, q, scale): return self.emit("dequantize_int8_per_channel", [q, scale])
    def quantize_pack_int8(self, x, *, dim=-1): return self.emit("quantize_pack_int8", [x], kwargs={"dim": int(dim)})
    def unpack_dequantize_int8(self, blob, *, shape, dim=-1): return self.emit("unpack_dequantize_int8", [blob], kwargs={"dim": int(dim), "shape": list(shape), "scale_dim": int(dim)})
    def qr(self, a): return self.emit_multi("qr", [a], n_outputs=2)
    def data_indexer(self, tokens, *, B, T, mb_seed): return self.emit_multi("data_indexer", [tokens], kwargs={"B": int(B), "T": int(T), "mb_seed": mb_seed}, n_outputs=2)

    def build(self) -> Graph:
        return Graph(inputs=list(self._inputs), outputs=list(self._outputs), params=list(self._params), ops=list(self._ops), ir_version=1)
